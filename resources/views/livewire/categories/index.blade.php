<div>
    @include('layout.partials.filters.name')
        ...
        <table class="table table-bordered table-hover dataTable" id="kt_datatable">
            ...
            @foreach($categories ?? [] as $category)
                <tr>
                    ...
                    <td nowrap>
                        @if($deleted)
                            <button onclick="loading()" wire:click="restore({{$category->id}})"
                        ...
                        @else
                            ...
                            @endcan
                            @can('delete', $category)
                                <button onclick="loading()" wire:click="delete({{$category->id}})"
                                        class="btn btn-sm btn-clean btn-icon" title="@lang('text.Disable')">
                                    <span class="svg-icon svg-icon-md">
	                                <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"
                                         width="24px" height="24px" viewBox="0 0 24 24" version="1.1">
	                                    <g stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
